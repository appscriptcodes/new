...
function TransactionsPage({ data, isAdmin, onRefresh, chartOfAccounts }) {
  ...
  const availableYears = useMemo(() => {
    ...
    data.forEach(t => {
      if (t.Date) {
        // handles both "2025-01-11" and "11-Jan-2025" formats
        const iso = normDate(t.Date);
        if (iso && iso.length >= 4) years.add(iso.substring(0, 4));
      }
    });
  ...
  const availableMonths = useMemo(() => {
    ...
    data.forEach(t => {
      if (t.Date) {
        ...
        if (!filterYear || y === filterYear) months.add(m);
      }
    });
  ...
  const filtered = useMemo(() => {
    ...
    if (filterYear || filterMonth) {
      r = r.filter(t => {
        ...
        if (filterYear  && iso.substring(0, 4) !== filterYear)  return false;
        if (filterMonth && iso.substring(5, 7) !== filterMonth) return false;
        return true;
      });
    }
        ...
        <div className="p-4 border-b border-gray-100 flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
              ...
              <select value={filterMonth}
                ...
                className="px-3 py-1.5 border border-gray-200 rounded-lg text-sm bg-white text-gray-700 disabled:opacity-50">
