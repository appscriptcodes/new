...
function TransactionsPage({ data, isAdmin, onRefresh, chartOfAccounts }) {
  ...
  /* ── Helper: Internal normalizer for filtering ── */
  const getParts = (dateVal) => {
    const normalized = normDate(dateVal); // Uses your existing normDate "YYYY-MM-DD"
    if (!normalized || normalized.length < 7) return { y: '', m: '' };
    return {
      y: normalized.substring(0, 4),
      m: normalized.substring(5, 7)
    };
  };
  ...
  const availableMonths = useMemo(() => {
    ...
    data.forEach(t => {
      ...
      if (y && (!filterYear || y === filterYear)) {
        if (m) months.add(m);
      }
    });
    return Array.from(months).sort().map(m => ({ value: m, label: MONTH_NAMES[parseInt(m, 10) - 1] }));
  }, [data, filterYear]);
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <select
                ...
                disabled={availableMonths.length === 0 && !filterYear}
                className="px-3 py-1.5 border border-gray-200 rounded-lg text-sm bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-300 disabled:opacity-50"
              >
            ...
            <div className="flex items-center gap-2 mt-3 flex-wrap">
              ...
              {typeFilter && (
                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium border ${typeFilter==='credit'?'bg-emerald-50 border-emerald-200 text-emerald-700':'bg-red-50 border-red-200 text-red-700'}`}>
                  {typeFilter === 'credit' ? '↑ Credits' : '↓ Debits'}
              ...
              <span className="text-xs text-gray-400 ml-1">— {filtered.length} result(s)</span>
            </div>
          )}
        </div>
