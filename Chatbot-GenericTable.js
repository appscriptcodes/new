...
function Chatbot({ data, setView, isAdmin, theme, onClose }) {
  ...
  React.useEffect(() => {
    const move = (e) => {
      ...
      const newX = Math.max(10, Math.min(window.innerWidth - width - 10, e.clientX - dragRef.current.offsetX));
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await postPlain({
        op: 'chat',
        message: userInput,
      ...
      setMessages(prev => [
        ...
        {
          id: Date.now() + 1,
          text: response.ok && response.reply ? response.reply : getLocalResponse(userInput),
          sender: 'bot'
        }
      ]);
    } catch (err) {
  ...
  return (
    <>
      {isOpen && (
        <div
          className={`chatbot-box fixed w-[420px] max-h-[600px] backdrop-blur-xl rounded-3xl shadow-2xl z-[100] flex flex-col border-2 ${
            theme === 'dark'
          ...
          style={
            pos.x === null
              ? { bottom: "7rem", right: "1.5rem", position: "fixed" }
              : {
                  left: `${pos.x}px`,
                  top: `${pos.y}px`,
                  right: "auto",
        ...
        >
          <div
            ...
            className="p-5 border-b flex items-center justify-between rounded-t-3xl bg-gradient-to-r from-blue-500 to-purple-600 cursor-grab active:cursor-grabbing select-none"
          >
          ...
          <div className="p-4 border-t border-gray-200/50">
            <div className="flex gap-2">
              <input
                ...
                className={`flex-1 px-4 py-3 border rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-sm ${
                  theme === 'dark'
                    ? 'border-gray-600/50 bg-gray-700/50 text-white placeholder-gray-400'
                    : 'border-gray-300/50 bg-white/50 text-gray-900 placeholder-gray-500'
                }`}
                disabled={loading}
              />
            ...
            <p className="text-xs text-gray-500 text-center mt-2">
              💡 Drag the top bar to move chatbot
            </p>
          </div>
        </div>
      )}
