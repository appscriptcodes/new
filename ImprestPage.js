...
function ImprestFormModal({ row, nextSr, currentMonth, onClose, onSave }) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await onSave({ ...form, amount: parseFloat(String(form.amount).replace(/[₹,\s]/g,'')) || 0 });
          ...
          <dl className="space-y-3">
            {fields.map(([label, value]) => (
              <div key={label} className="flex justify-between gap-4">
                ...
                <dd className={`text-sm font-medium text-right break-words max-w-xs ${label === 'Amount' ? 'text-amber-600 text-base' : 'text-gray-700'}`}>{value}</dd>
              </div>
            ))}
          </dl>
        </div>
...
function ImprestPage({ gasData, onDataChange }) {
  ...
  const handleAdd = useCallback(async (formData) => {
    setSaving(true);
    try {
      await api.createRow('Imprest', toSheetRow(formData));
      const res = await api.list.Imprest();
      onDataChange(res?.rows || []);
  ...
  const handleEdit = useCallback(async (formData) => {
    setSaving(true);
    try {
      await api.upsertRow('Imprest', 'Sr No', toSheetRow(formData));
      const res = await api.list.Imprest();
      onDataChange(res?.rows || []);
