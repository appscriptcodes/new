...
function FlatsAdjuster({ occupiedA, occupiedB, setOccupiedA, setOccupiedB }) {
  ...
  return (
    <div className="bg-white rounded-xl p-5 shadow-sm">
      ...
      <div className="space-y-4">
        {[
          { label: 'Type A Flats', max: maxA, val: occupiedA, set: setOccupiedA, color: '#3b82f6' },
          { label: 'Type B Flats', max: maxB, val: occupiedB, set: setOccupiedB, color: '#6366f1' },
        ].map(f => (
          <div key={f.label}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium text-gray-600">{f.label}</span>
              <span className="text-xs text-gray-400">{f.val} / {f.max} paying</span>
            </div>
            ...
            {f.val < f.max && (
              <p className="text-xs text-amber-600 mt-1">
                ⚠️ {f.max - f.val} flats not paying · Loss: {fmt((f.max - f.val) * (AREA_A * 2.57 + 300 + 60))}/month
              </p>
            )}
          </div>
        ))}
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm">
          <div className="flex justify-between">
            ...
            <span className="font-bold text-amber-700">{occupiedA + occupiedB} / {maxA + maxB}</span>
          </div>
          <div className="flex justify-between mt-1">
            <span className="text-gray-600">Non-paying flats</span>
            <span className="font-bold text-red-500">{(maxA + maxB) - (occupiedA + occupiedB)}</span>
          </div>
        </div>
      </div>
    </div>
...
function BudgetPage() {
  ...
  const expenseGroups = [
    { label:'Staffing',    ids:['security','enviro','garbage'],                                    color:'#3b82f6' },
    { label:'Maintenance', ids:['lifts','dgset','transformer','fire','boombarrier','smartmeters'], color:'#f59e0b' },
    { label:'Operations',  ids:['stp','diesel','plumbing','accidental','pettycash'],               color:'#10b981' },
    { label:'Compliance',  ids:['govt'],                                                           color:'#8b5cf6' },
          ...
          <div className="bg-white rounded-xl p-5 shadow-sm">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-gray-50">
                    {['CAM Rate','Type A CAM','Type B CAM','Water','Elec Fixed','Elec Var','Type A Total','Type B Total'].map(h => (
            ...
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl p-5 text-white">
              ...
              <div className="space-y-3 text-xs">
                {[
                  ...
                  [`Security = ${((expenses.find(e=>e.id==='security')?.amount||0)/totalExpenses*100).toFixed(1)}% of total expenses`, '#f87171'],
                ].map(([text,color],i) => (
